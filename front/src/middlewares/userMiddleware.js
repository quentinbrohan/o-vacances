...
const userMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case SIGN_IN: {
      const { firstname, lastname, email, password } = store.getState().user;
    ...
    case FETCH_USER: {
      ...
      axios
        .get(`${API_URL}/api/v0/users/${currentUser()}/profil`)
        .then((response) => {
          // console.log(response);
    ...
    case EDIT_USER: {
      const { email, lastname, firstname, password } = store.getState().user;

      // withCredentials : autorisation d'accéder au cookie
      axios
        .patch(`${API_URL}/api/v0/users/${currentUser()}/edit`, {
          email,
          lastname,
          firstname,
          password,
    ...
    case EDIT_USER_IMAGE: {
      const imageInput = document.querySelector(
        "#profile-field-input.profile-image"
      );
      ...
      axios
        .put(
          `${API_URL}/api/v0/users/${currentUser()}/upload`,
          formData,
          config
        )
        .then((response) => {
